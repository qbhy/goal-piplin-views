      ...
      >
        <ProForm
          ...
          onFinish={async (values: Record<string, any>) => {
            setLoading(true);
            (config?.id ? updateCommand : createCommand)({ ...values, project_id: projectId }).then(
              () => {
                setLoading(false);
                setForm(undefined);
                tableRef.current?.reload();
        ...
        >
          ...
          <ProFormText required name="sort" label="排序" initialValue={1} tooltip="越小越靠前" />
          <ProFormGroup>
            <ProFormSwitch name="optional" label="是否可选" />
            <ProFormSwitch name="default_selected" label="是否默认选中" />
          </ProFormGroup>
          <ProFormTextArea required name="script" label="部署脚本" placeholder="请输入脚本" />
        </ProForm>
      </Modal>
